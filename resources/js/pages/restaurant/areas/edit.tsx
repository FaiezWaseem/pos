...
export default function AreaEdit({ area, restaurants }: Props) {
    const { auth } = usePage<SharedData>().props;
    const isSuperAdmin = auth.user?.role?.name === 'super_admin';

    const { data, setData, put, processing, errors } = useForm({
        restaurant_id: area.restaurant_id.toString(),
        name: area.name,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight">Edit Area</h2>
                        <p className="text-muted-foreground">Update floor area details.</p>
                    </div>
                </div>
